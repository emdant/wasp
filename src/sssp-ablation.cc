...
auto NoOptDS(const WGraph& g, NodeID source, int32_t delta) {
...
auto BidirectionalRelaxDS(const WGraph& g, NodeID source, int32_t delta) {
  int num_threads = omp_get_max_threads();
  parallel::atomics_array<WeightT> dist(g.num_nodes(), DIST_INF);
  dist[source] = 0;
  executor scheduler;
...
auto NeighDecompositionDS(const WGraph& g, NodeID source, int32_t delta) {
...
template <bool DIRECTED, bool CACHE_LEAVES>
auto LeavesDS(const WGraph& g, NodeID source, int32_t delta) {
  int num_threads = omp_get_max_threads();
  parallel::atomics_array<WeightT> dist(g.num_nodes(), DIST_INF);
  dist[source] = 0;
  executor scheduler;
...
parallel::atomics_array<WeightT> DeltaStep(const WGraph& g, NodeID source, int32_t delta) {
  // No Optimizations
  return NoOptDS(g, source, delta);

  // Only Neigh-Decomposition
  // return NeighDecompositionDS(g, source, delta);

  // Only Bidirectional Relaxation
  // if (g.directed())
  //   return BidirectionalRelaxDS<true>(g, source, delta);
  // else
  //   return BidirectionalRelaxDS<false>(g, source, delta);

  // Only Leaves Pruning
  // if (g.directed())
  //   return LeavesDS<true, true>(g, source, delta);
  // else
